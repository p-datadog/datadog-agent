...
static long sm_loop(__maybe_unused unsigned long i, void* _ctx) {
  ...
  case SM_OP_SWISS_MAP_LOOP_BEGIN: {
    ...
    //
    // Re-entering this op via `sm->pc -=` (the back-edge retry below)
    // re-reads these same bytes into the same fields — that's idempotent.
    // The phase flag below distinguishes the first entry (full init) from
    // back-edge retries (skip init, just step again).
    ...
    // step_r == 2: empty slot, keep stepping. Re-enter the same op via
    // `sm->pc -=`. Begin's total encoded length is 1 (opcode) + 1 + 4 + 4 +
    // 5*u8 + 2 + 2 + 2 + 3*u8 + 4 = 28 bytes.
    sm->pc -= 28;
  } break;
  ...
  case SM_OP_SWISS_MAP_LOOP_END: {
    ...
    // step_r == 2: empty slot, keep stepping. Re-enter the same op via
    // `sm->pc -=`. End's total encoded length is 1 (opcode) + 4 (body_label) = 5.
    sm->pc -= 5;
  } break;
